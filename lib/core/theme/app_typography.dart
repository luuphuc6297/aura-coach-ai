...
///
/// **Color policy:** these constants intentionally do NOT hardcode a color.
/// `Text(style: AppTypography.h1)` merges the style with the closest
/// `DefaultTextStyle` ancestor (which Material derives from the active
/// `ThemeData.textTheme` + `colorScheme.onSurface`). Light theme resolves to
/// the warmDark text color; dark theme resolves to cream — automatically.
///
/// Callers that need a specific (e.g. muted) color still apply
/// `.copyWith(color: context.clay.textMuted)` at the call site; that path is
/// unchanged.
